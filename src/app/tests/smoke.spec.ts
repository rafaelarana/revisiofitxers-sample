...
// ── RevisioFitxers app config ───────────────────────────────────────────────
const APP_CONFIG = { name: 'RevisioFitxers' } as const;
...
test('smoke test - app loads and displays expedients page', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: APP_CONFIG.name })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Expedients' })).toBeVisible();
  // Search box is always present, even with zero expedients.
  await expect(page.getByPlaceholder(/Cerca per ID/)).toBeVisible();
});
